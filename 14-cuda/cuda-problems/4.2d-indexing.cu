...
__global__ void add(float *A, float *B, float *C, int width, int height){
    int col = blockIdx.x * blockDim.x + threadIdx.x;  // x-coordinate (column) 0-2
    int row = blockIdx.y * blockDim.y + threadIdx.y;  // y-coordinate (row) 0-1
...
int main() {
    ...
    dim3 numBlocks(2, 2);                 // 2 x 2 blocks
